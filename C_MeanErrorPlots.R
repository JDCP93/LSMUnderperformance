# Code to produce the supplementary figures that plot the actual errors of the 
# LSMs relative to observations

rm(list = ls())

# User-defined variables
DaytimeFlags = F #c(FALSE, TRUE)
PhysicalFlags = F #c(FALSE, TRUE)
WindyFlags = F #c(FALSE, TRUE)
    ...
    for (WindyFlag in WindyFlags){
      for (var1 in variables){
        ...
        message(Sys.time(),
                ": BEGIN - ",
                Flux,
                ifelse(DaytimeFlag, " - Daytime",""),
                ifelse(PhysicalFlag, " - Physical",""),
                ifelse(WindyFlag," - Windy",""),
                " ",
                var1)
        ...
        plotname <- paste0("plots/MeanErrorPlots/",
                           Flux,
                           flags,
                           "_",
                           var1,
        ...
        if (file.exists(lossdataname) & file.exists(plotname)){
          ...
          FluxUnit <- c("(W/m<sup>2</sup>)", "(W/m<sup>2</sup>)","(µmol/m<sup>2</sup>s)")[Flux==fluxes]
          FluxSigma <- c(25, 25, 2.5)[Flux==fluxes]
        ...
        for(var2 in setdiff(variables, var1)){
          ...
          i = i + 1
            ...
            metric_names = c("Error" = "LSM Mean Error",
                             "BinNo" = "Timesteps in Cell")
            
            # Arrange error data
            setnames(Data_modified, var1, "var1")
            setnames(Data_modified, var2, "var2")
            basedata <- data.table::melt(Data_modified,
                                         measure.vars = patterns("error"),
                                         variable.name = "Model",
                                         value.name = "ModelError",
                                         variable.factor = FALSE
            )[, Model := sub("_error","",Model)
            ][, .(
              Error = mean(ModelError, na.rm = TRUE)
            ),
            by = c("Model", "var1", "var2")
            ]
            
            # Include the root mean square of LSM errors as an aggregate
            RMSmodeldata <- basedata[,
                             .(Error = sqrt(mean((Error^2), na.rm = TRUE))),
                             by = .(var1, var2)
                             ][,Model := "bold(atop('All LSMs','(RMSE)'))"]
            
            # Also useful if we can see where the majority of timesteps are located
            timestepdata <- Data_modified[,
                                          .(Error = .N * 100 / Timesteps),
            ...
            plotdata <- rbind(basedata, timestepdata, RMSmodeldata) %>%
              filter(!(Model %in% c("1lin","3km27","6km729lag","RF","LSTM")))
            
            plot <- ggplot(plotdata) +
              geom_tile(aes(x = var1, y = var2, fill = Error)) +
              scale_fill_binned_divergingx(name = paste0("Mean LSM Error for ",FluxTitle," in Cell ",FluxUnit),
                                           palette = 'RdBu', 
                                           mid = 0, 
                                           rev = TRUE,
                                          n.breaks = 31,
                                          # We want pseudo-log due to the distribution of errors
                                          transform=scales::pseudo_log_trans(sigma = FluxSigma)) +
